...
def _retry_supabase_operation(operation_func, max_retries=3, base_delay=0.5):
    ...
    Handles connection timeouts, HTTP/2 protocol errors, and temporary failures.
    """
    last_exception = None
    ...
    for attempt in range(max_retries):
        try:
            ...
            is_retryable_error = any(keyword in error_msg for keyword in [
                ...
                'protocol', 'stream', 'localprotocol', 'remoteprotocol',
                'connectionterminated', 'streamclosed'
            ]) or any(keyword in error_type for keyword in [
                'localprotocol', 'remoteprotocol', 'connectionterminated',
            ...
            logger.warning(f"Supabase operation failed (attempt {attempt + 1}/{max_retries}): {type(e).__name__}: {e}. Retrying in {delay:.2f}s...")
            time.sleep(delay)
...
def get_project_by_code(project_code: str):
    ...
    response = _retry_supabase_operation(_fetch_project)
...
def get_stages_by_project(project_code: str):
    ...
    # Use retry mechanism for this operation
    try:
        response = _retry_supabase_operation(_fetch_stages)
        return list({item["phase"] for item in response.data})
    except Exception as e:
        logger.error(f"Error en get_stages_by_project: {e}", exc_info=True)
        raise
...
def get_projects():
    ...
    # Use retry mechanism for this operation
    return _retry_supabase_operation(_fetch_projects)
...
def get_employees():
    ...
    # Use retry mechanism for this operation
    return _retry_supabase_operation(_fetch_employees)
...
def get_member_by_id(member_id: int):
    ...
    # Use retry mechanism for this operation
    response = _retry_supabase_operation(_fetch_member)
...
def get_all_activities():
    ...
    # Use retry mechanism for this operation
    return _retry_supabase_operation(_fetch_all_activities)
...
def get_disciplines_by_stage(project_code: str, stage: str):
    ...
    # Use retry mechanism for this operation
    try:
        response = _retry_supabase_operation(_fetch_disciplines)
        return list({item["discipline"] for item in response.data})
    except Exception as e:
        logger.error(f"Error en get_disciplines_by_stage: {e}", exc_info=True)
        raise
...
def get_activities_by_discipline(project_code: str, stage: str, discipline: str):
    ...
    # Use retry mechanism for this operation
    try:
        return _retry_supabase_operation(_fetch_activities)
    except Exception as e:
...
def get_user_by_username(username: str):
    ...
    # Use retry mechanism for this operation
    response = _retry_supabase_operation(_fetch_user)
...
def get_activity_id(project_code: str, phase: str, discipline: str, activity: str) -> int:
    ...
    # Use retry mechanism for this operation
    return _retry_supabase_operation(_search_activity)
...
def create_reported_hour(hour: schemas.ReportedHourCreate):
    ...
    # 5. Insertar en la base de datos con retry
    def _insert_hour():
    ...
    row = _retry_supabase_operation(_insert_hour)
...
def update_reported_hour(hour_id: str, hour_update: schemas.ReportedHourUpdate):
    ...
    # Use retry mechanism for this operation
    try:
        return _retry_supabase_operation(_update_hour)
    except Exception as e:
...
def delete_reported_hour(hour_id: str):
    ...
    # Use retry mechanism for this operation
    try:
        return _retry_supabase_operation(_delete_hour)
    except Exception as e:
...
def get_daily_activities(date: str, employee_id: int):
    ...
    # Use retry mechanism for this operation
    try:
        return _retry_supabase_operation(_fetch_daily_activities)
    except Exception as e:
...
def get_grouped_hours_by_employee(year: int, month: int):
    ...
    # Use retry mechanism for this operation
    try:
        return _retry_supabase_operation(_fetch_grouped_hours)
    except Exception as e:
        logger.error(f"Error en get_grouped_hours_by_employee después de reintentos: {e}", exc_info=True)
        raise
...
def update_user_password(username: str, new_password_hash: str):
    ...
    # Use retry mechanism for this operation
    try:
        return _retry_supabase_operation(_update_password)
    except Exception as e:
